<script>
export default { name: "ProductCategoriesForm" }
</script>
...
<template>
    <FormSection @submitted="$emit('submit')">
        ...
        <template #title>
            {{ updating ? __('Edit') + " " + __('product_category').toLowerCase() : __('Create') + " " +
        __('product_category').toLowerCase() + "" }}
        </template>
        <template #description>
            {{ updating ? __('txt_edit_product_category') : __('txt_new_product_category') }}
        </template>
        <template #form>
            <div class="col-span-6 sm:col-span-6 mb-5">
                ...
                <InputLabel for="name" name="Name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    {{ __('product_category_name_label') }}</InputLabel>
